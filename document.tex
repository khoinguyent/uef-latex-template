...
\begin{equation} \label{Taylor Series Appx.}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $p0 \gets find \ interesting \ points$ \Comment {using corner detection}
        \State $mask \gets create \ a \ mask \ image$
        \While {reading video frames}
            ...
            \LineComment{Update previous frame and points}
...
\begin{equation}\label{quad.polynomial}
    f_{1}(x) = x^TA_{1}x + b_{1}^Tx + c_{1}
\end{equation}
\myequations{Quadratic Polynomial}

And with the global displacement factor \textbf{d}, the signal $f_{2}$ is created following (\ref{f2 signal})

\begin{equation}\label{f2 signal}
    \begin{split}
        f_{2}(x) & = f_{1}(x - d) \\
                & = (x - d)^TA_{1}(x - d) + b_{1}^T(x - d) + c_{1} \\
                & = x^TA_{1}x + (b_{1} - 2A_{1}d)^Tx + (d^TA_{1}d - b_{1}^Td + c{1})\\
                & = x^TA_{2}x + b_{2}^Tx + c_{2}
    \end{split}
\end{equation}
\myequations{New signal $f_{2}$ by global displacement}

Thus

\begin{equation} \label{coef.1}
    A_{2} = A_{1}
\end{equation}
\myequations{Polynomial expansion coefficient 1}

\begin{equation} \label{coef.2}
    b_{2} = b_{1} - 2A_{1}d
\end{equation}
\myequations{Polynomial expansion coefficient 2}

\begin{equation} \label{coef.3}
    c_{2} = d^TA_{1}d - b_{1}^Td + c{1}
\end{equation}
\myequations{Polynomial expansion coefficient 3}

It can be seen that the translation \textbf{d} can be solved at least \textbf{$A_{1}$} is non-singular by the equation (\ref{coef.2}) \citep{farneback2003two}

\begin{equation}
    2A_{1}d = -(b_{2} - b_{1})
    \Rightarrow d = -\frac{1}{2}A_{1}^{-1}(b_{2} - b_{1})
\end{equation}
\myequations{The key observation}

The Farneback optical flow algorithm can be explained following the steps:
\begin{itemize}
    \item The windows of image frames is approximated by quadratic polynomials follow polynomial expansion transform as above.
    \item Based on the polynomial transform under motion, estimation of displacement fields is defined from polynomial expansion coefficients. Then dense optical flow is calculated after some refinements (in the scope of this thesis, the details of calculation steps are not referred).
\end{itemize}

To visualize the optical flow with OpenCV, from the 2 channels array of flow vectors ($\frac{dx}{dt} and \frac{dy}{dt}$) (from optical flow problem (\ref{Op Flow equa})), the magnitude and direction (angle) of flow vectors are computed (2D vector) \citep{openCvArrayFunc}.

\begin{equation}\label{magnitude}
    magnitude(I) = \sqrt{x(I)^2 + y(I)^2}
\end{equation}
\myequations{Magnitude calculation}

\begin{equation}\label{angle}
    angle(I) = \arctantwo{(y(I), x(I))}\frac{180}{\pi}
\end{equation}
\myequations{Angle of two 2D vectors}

The below pseudocode is show how conduct an optical frames from a RGB video.

\begin{algorithm}[H]
    \caption{Convert into dense optical flow video}
    \begin{algorithmic}[1]
        \Require $video\_input$
        \State $cur\_frame \gets first \ video \ frame$
        \State $cur\_frame \gets convert \ \textit{frame} \ into \ grayscale \ frame $
        \State $hsv\_mask \gets matrix \ 0 \ with \ size \ of \ \textit{frame}$
        \State $hsv\_mask \gets set \ image \ saturation \ to \ a \ maximum \ value \ 255$
        \While {reading video frames}
            \State $prev\_frame \gets get \ previous \ grayscale \ frame$
            \State $cur\_frame \gets current \ frame$
            \State $cur\_frame \gets convert \ cur\_frame \ into \ grayscale$
            \State $flow \gets calculate \ optical \ flow \ based \ on \ prev\_frame \ and \ cur\_frame$ \Comment{usingcv2.calcOpticalFlowFarneback}
            \State $magnitude,\ angle \gets calculate \ flow \ vectors \ magnitude \ and \ angle$ \Comment{using cv2.cartToPolar}
            \LineComment{Set value as per the normalized magnitude of optical flow}
            \State $hsv\_mask \gets angle * 180 \ / \ \mathit{\pi} \ / \ 2$
            \LineComment{Set value as per the normalized magnitude of optical flow}
            \State $hsv\_mask \gets nomalized \ magnitude$
            \State $rgb\_representation \gets convert \ hsv\_mask \ from \ HSV \ to \ RGB$ 
        \EndWhile
    \end{algorithmic}
\end{algorithm}

This algortithm is used in the thesis. However, for the flow system, it is not needed to represent flow frames therefore to generate flow tensor input, the process is stopped at generating flow vectors \textit{(u, v)} by using function \textit{calcOpticalFlowFarneback} which is supported by OpenCV. So the size of flow tensonr is \textit{$(num\_frames, heigh, width, 2)$}, the number 2 indicates 2 channels of flow image, while the input size of RGB tensor is \textit{$(num\_frames, heigh, width, 3)$}, 3 represents 3 channel RGB of an image.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Multi-class vs multi-label.png}
    \caption{Multi-class vs Mult-label classification}
    \label{fig. Multi-class vs multi-label}
\end{figure}

Generally, the output vectors of CNNs are transformed to compute loss value by Sigmoid or Softmax functions. When applied Sigmoid function, the vector is scaled in range of 0 to 1, and for each element, it is calculated independently. Softmax function, it is not a loss, is used to produce scores \textit{\textbf{$s$}}, in which an element is a probability of the prediction belongs to the class \textit{\textbf{$s_i$}}. The Softmax function depends on all elements of \textit{\textbf{$s$}}. The Sigmoid (\ref{Sigmoid}) and Softmax (\ref{Softmax}) function can be calculated as equations below \citep{understandingLossFunction}.

\begin{equation} \label{Sigmoid}
    f(s_i) = \frac{1}{1 + e^{-s_i}}
\end{equation}
\myequations{Sigmoid function}
...
To measure the difference between to probability distributions of set of data, cross-entropy is applied. The cross-entropy loss is defined:

\begin{equation} \label{Cross-Entropy calculation}
...
To evaluate the model of multi-class classification, Softmax activaton combined with Cross-Entropy loss is used. It means the output of CNNs is a probability over the C classes for each input item.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Softmax Cross-Entropy Loss.png}
    \caption{Softmax Cross-Entroy Loss}
\end{figure}

\begin{equation} \label{Softmax Cross-entropy loss (a)}
    CE = -\sum_{i}^{C}t_{i}log(\frac{e^{s_i}}{\sum_{j}^{C}e^{s_j}})
\end{equation}
\myequations{Softmax Cross-entropy Loss function (a)}

As normally, in case of multi-class problems, labels are reprented as one-hot vectors, thus only the class $C_p$ (positive class) keeps its term in the loss. So far, only one element in the target vector is non-zero, $t_i = t_p$. Because $t_j = 0$ when $j \neq i$ and $t_i = 1$ when $i = p$, therefore the equation (\ref{Softmax Cross-entropy loss (a)}) can be written as

\begin{equation} \label{Softmax Cross-entropy loss (b)}
    CE = -log(\frac{e^{s_p}}{\sum_{j}^{C}e^{s_j}})
\end{equation}
\myequations{Softmax Cross-entropy Loss function (b)}

The next step is find out the optimum values for parameters in the network by calculating the gradient of loss function with respect to the output of CNN, then applying the back-propagation algorithm to optimize the loss function. So the gradient of CE loss respect each class score in $s$ needs to be computed. Despite the groundtruth $C_p$, the gradient for other classes are the same, the derivative respect to the $C_p$ class (eq. (\ref{Derivative for positive class C_p})) and other classes (eq. (\ref{Derivative for negative classes})) are defined \citep{cross-entropy,softmax-derivative}:

\begin{equation} \label{Derivative for positive class C_p}
    \frac{\delta}{\delta s_{p}}(-log(\frac{e^{s_p}}{\sum_{j}^{C}e^{s_j}})) = (\frac{e^{s_p}}{\sum_{j}^{C}e^{s_j}} - 1)
\end{equation}
\myequations{Derivative for positive class}

\begin{equation} \label{Derivative for negative classes}
    \frac{\delta}{\delta s_{n}}(-log(\frac{e^{s_p}}{\sum_{j}^{C}e^{s_j}})) = (\frac{e^{s_n}}{\sum_{j}^{C}e^{s_j}})
\end{equation}
\myequations{Derivative for negative classes}

$s_n$ is the score of negative classes differ from $C_p$

\subsubsection{Binary Cross-Entropy loss}
Instead of using a combiation between Softmax activation and cross-entropy, this loss function is using Sigmoid activation. Therefore it is also called as Sigmoid Cross-entropy. While in Softmax activation a score of an elements depends on all elements, the output with Sigmoid activation are dependently to each others so that the loss estimated value for each output vector component is unaffected by the value of other component elements. This is the reason why Sigmoid Cross-Entropy is used in multi-label classification problems where prediction of an element belonging to a specific class should not affect the judgement for another class.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Binary Cross-entropy.png}
    \caption{Binary Cross-Entropy Loss}
    \label{fig. binary CE loss}
\end{figure}

In this case, an output is evaluated that it is belonging to the class $C_i$ (for all classes $C$) or not, tha's why it is called \textit{"binary"}. The local loss for each class $C_i$ is defined (\ref{Binary Cross-Entropy Loss})

\begin{equation} \label{Binary Cross-Entropy Loss (a)}
    \begin{split}
        CE & = -\sum_{i = 1}^{C' = 2} t_{i}log(f(s_i)) \\
            & = -t_{1}log(f(s_1)) - (1 - t_1)log(1 - f(s_1))
    \end{split}   
\end{equation}
\myequations{Binary Cross-Entropy Loss (a)}

Where $f(s_{i})$ is sigmoid function at (\ref{Sigmoid}). The problem of multi-label classification can be considered as multi \textit{small} binary problems: is the output belonged to the class $C_i$ for every class $C$? Therefore each $C_i$ class, $t_1$ and $s_1$ is the groundtruth and the score of the setup binary problem within the class. It is known that the value of $t_1$ is binary (only 0 or 1, positive or negative) so the equation (\ref{Binary Cross-Entropy Loss (a)}) can re-write as

\begin{equation} \label{Binary Cross-Entropy Loss (b)}
    f(x) = 
    \begin{dcases}
        -log(f(s_{1})) \text{if $t_1 = 1$} \\
        -log(1 - f(s_1)) \text{if $t_1 = 0$}
    \end{dcases}
\end{equation}
\myequations{Binary Cross-Entropy Loss (b)}
...
\begin{equation} \label{binary cross-entropy gradient}
    \begin{split}
        \frac{\delta}{\delta s_i}(CE(f(s_i))) & = t_1(f(s_1) - 1) + (1 - t_1)(f(s_1)) \\
        & = \begin{dcases}
            f(s_1) - 1 \text{if $t_1 = 1$} \\
            f(s_1) \text{if $t_1 = 0$}
        \end{dcases}
    \end{split}
\end{equation}
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|c|c|c|c|c|}
            \hline
            \textbf{Dataset} & \textbf{\#Gloss} & \textbf{\#Videos} & \textbf{Mean} & \textbf{\#Signers} \\ 
            \hline WLASL100 & 100 & 2038 & 20.4 & 97 \\ 
            \hline WLASL300 & 300 & 5117 & 17.1 & 109 \\ 
            \hline WLASL1000 & 1000 & 13168 & 13.2 & 116 \\ 
            \hline WLASL2000 & 2000 & 21083 & 10.5 & 119 \\ 
            \hline
        \end{tabular}%
    }
